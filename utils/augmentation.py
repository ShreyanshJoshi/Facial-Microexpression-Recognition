...
from utils.miscellaneous import get_frames_3dimages
...
def augment_helper_3dimages(frames):
    img_arr1 = frames[0]
    img_arr1_1 = frames[1]
    img_arr1_2 = frames[2]
    img_arr1_3 = frames[3]
    ...
    final = np.concatenate((img_arr1, img_arr1_1, img_arr1_2, img_arr1_3, img_arr2, img_arr2_1, 
                                img_arr2_2, img_arr2_3, img_arr3), axis=0)
    return final
...
def augment_3dimages(classes, type):
    ...
    if type=="multiclass":        
        for i in classes[0]:
            frames = get_frames_3dimages(frames, i)
            final = np.concatenate((frames[0], frames[1], frames[2], frames[3], frames[4], frames[5], 
                                    frames[6], frames[7], frames[8]), axis=0)
            data.append(final)
            labels.append(0)

            for _ in range(1):
                final = augment_helper_3dimages(frames)
                data.append(final)
                labels.append(0)
...
'''A helper function, used in the case when all 9 frames are considered as individual datapoints during training (majority voting with 2D CNNs). This function randomly generates parameters for augmentation, and decides randomly which type of augmentation has to be performed, and finally returns the augmented image (array). '''
def augment_helper_2dimages(img_arr):
    directions = ['right', 'left']
    dir_index = random.randint(0,1)
    ...
    while flag==0:
        ...
        if coin==1:
            flag = 1
            img_arr = Augmentation.translate(img_arr, shift=shift, direction=directions[dir_index], model_name="2dcnn")

        coin = random.randint(0,1)        # Rotation
        if coin==1:
            flag = 1  
            img_arr = Augmentation.rotate_img(img_arr, angle[angle_index], model_name="2dcnn")

        coin = random.randint(0,1)        # Gaussian noise
        if coin==1:
            flag = 1   
            img_arr = Augmentation.gaussian_noise(img_arr, mean=mean, sigma=sigma, model_name="2dcnn") 
...
def augment_add_to_list_2dimages(train_labels, train_data, j, i):
    train_labels.append(i[0])
    img_arr = augment_helper_2dimages(img_to_array(load_img(dir + j, color_mode="grayscale", target_size=(128,128))))
    train_data.append(np.array(img_arr))
...
def augment_2dimages(train_list, type, augment=False):
            ...
            for j in i[1]:
                ...
                if type=="multiclass":
                    ...
                    if coin==1:                             # Whether or not to augment is decided by the flip of a coin
                        if i[0]==6:
                            augment_add_to_list_2dimages(train_labels, train_data, j, i)
                            augment_add_to_list_2dimages(train_labels, train_data, j, i)
                            augment_add_to_list_2dimages(train_labels, train_data, j, i)
                            augment_add_to_list_2dimages(train_labels, train_data, j, i)
                    ...
                    if i[0]==1:                            # If at all augmenting for binary classification, no need to augment negative images.
                        continue
