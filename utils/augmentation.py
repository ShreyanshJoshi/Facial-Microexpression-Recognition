...
# A helper function, used in the case when all 9 frames of a datapoint are stacked depthwise. It augments each of the 9 frames in a similar manner (since they would be stacked one behind the other), by randomly generating parameters for augmentation, and randomly deciding which type of augmentation has to be performed. It then stacks the augmented one behind the other (depthwise) and returns it.
...
# A helper function, used in the case when all 9 frames are considered as individual datapoints during training. This function randomly generates parameters for augmentation, and decides randomly which type of augmentation has to be performed, and finally returns the augmented image (array).
