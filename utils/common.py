...
class Augmentation():
    ...
    @staticmethod
    def translate(img, shift=10, direction='right', roll=True, model_name="3dcnn"):
        if img.ndim==4:
    ...
    '''A quite effective way to augment image is to rotate it a random degrees. The “empty” space in the corners has been filled with the mean of colours from 
    the corner-patch. '''
    @staticmethod
    def rotate_img(img, angle, bg_patch=(15,15), model_name="3dcnn"):
        if img.ndim==4:
            img = img.reshape(img.shape[1],img.shape[2],img.shape[3])
    ...
    @staticmethod
    def gaussian_noise(img, mean=5, sigma=1, model_name="3dcnn"):
        if img.ndim==4:
            img = img.reshape(img.shape[1],img.shape[2],img.shape[3])
        ...
        return noisy_img_clipped[newaxis,:,:,:]

    '''NOTE: Another interesting way to change original sample is to somehow distort it. As a simple example we can apply continuous shift of the rows or columns 
    of our image guided by trigonometric functions (cosine or sinus). The resulting image would be “wavy” in horizontal or vertical directions. By tuning function 
...
'''Returns 3 equally spaced frames between 2 frames (end_path1 and end_path2). '''
def get_next_three(end_path1, end_path2):             
    ...
    # Below, finding 3 equidistant numbers between number 1 and number 2
    x = int(number1)
    y = int(number2)
    add = (y-x)//4
...
'''Plots 2 plots after training, for both training and validation process -  epochs vs accuracy and epochs vs loss.'''
