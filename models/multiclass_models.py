...
def load_model_3dcnn():
    ...
    x = Conv3D(filters=32, kernel_size=(1,3,3), activation="relu", kernel_initializer='he_uniform')(inputs)
    x = MaxPool3D(pool_size=(1,2,2))(x)
    x = BatchNormalization()(x)

    x = Conv3D(filters=64, kernel_size=(1,3,3), activation="relu", kernel_initializer='he_uniform')(x)
    x = MaxPool3D(pool_size=(1,2,2))(x)
    x = BatchNormalization()(x)
