...
from model_architectures._2dcnn import load_model_binary
...
def main():
    ...
    '''Augmentation doesn't add value in this case as binary classification is intrinsically a not so difficult task and secondly, because we are treating each of 
    the 9 frames of a datapoint individually in this case, we have 9 times the datapoints available for training (as compared to the cases when we stacked the 9 
    frames depthwise to produce a single 3D image). Adding augmentation only introduces redundancy in the dataset.'''
